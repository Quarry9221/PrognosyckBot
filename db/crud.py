...
async def get_or_create_user(
    ...
    if not user:
        ...
        updated = False
        for attr, value in [('username', username), ('first_name', first_name),
...
async def create_default_weather_settings(session: AsyncSession, telegram_id: int) -> UserWeatherSettings:
...
async def get_user_weather_settings(session: AsyncSession, telegram_id: int) -> UserWeatherSettings:
...
async def update_user_location(
    ...
    await session.commit()
    logger.info(f"Оновлено локацію для користувача {telegram_id}: {location_name} ({latitude}, {longitude})")
...
async def toggle_display_setting(session: AsyncSession, telegram_id: int, setting_name: str) -> bool:
    settings = await get_user_weather_settings(session, telegram_id)
    if hasattr(settings, setting_name):
        ...
        settings.updated_at = datetime.now()
        await session.commit()
        logger.info(f"Перемкнуто {setting_name} для користувача {telegram_id}: {current_value} -> {new_value}")
        return new_value
    else:
        raise ValueError(f"Невідомий параметр: {setting_name}")

async def update_forecast_settings(session: AsyncSession, telegram_id: int,
                                   forecast_days: int = None, past_days: int = None) -> None:
    settings = await get_user_weather_settings(session, telegram_id)
...
async def update_notification_settings(session: AsyncSession, telegram_id: int,
                                       notification_enabled: bool = None, notification_time: str = None) -> None:
    settings = await get_user_weather_settings(session, telegram_id)
...
async def get_api_parameters(session: AsyncSession, telegram_id: int) -> Dict[str, Any]:
    ...
    if not settings.latitude or not settings.longitude:
        raise ValueError("Локація не встановлена. Спочатку вкажіть своє місцезнаходження.")
    ...
    # Hourly
    hourly_params = []
    if settings.show_temperature: hourly_params.append('temperature_2m')
    if settings.show_feels_like: hourly_params.append('apparent_temperature')
    if settings.show_humidity: hourly_params.append('relative_humidity_2m')
    if settings.show_pressure: hourly_params.append('pressure_msl')
    if settings.show_wind: hourly_params.extend(['wind_speed_10m','wind_direction_10m','wind_gusts_10m'])
    if settings.show_precipitation: hourly_params.extend(['precipitation','rain','showers'])
    if settings.show_precipitation_probability: hourly_params.append('precipitation_probability')
    if settings.show_cloud_cover: hourly_params.append('cloud_cover')
    if settings.show_uv_index: hourly_params.append('uv_index')
    if settings.show_visibility: hourly_params.append('visibility')
    if settings.show_dew_point: hourly_params.append('dew_point_2m')
    if settings.show_solar_radiation: hourly_params.extend(['shortwave_radiation','direct_radiation','diffuse_radiation'])
    hourly_params.extend(['weather_code','is_day'])
    if hourly_params: params['hourly'] = ','.join(list(set(hourly_params)))

    # Daily
    daily_params = ['weather_code']
    if settings.show_daily_temperature:
        daily_params.extend(['temperature_2m_max','temperature_2m_min','apparent_temperature_max','apparent_temperature_min'])
    if settings.show_daily_precipitation:
        daily_params.extend(['precipitation_sum','precipitation_probability_max','precipitation_hours'])
    if settings.show_daily_wind:
        daily_params.extend(['wind_speed_10m_max','wind_gusts_10m_max','wind_direction_10m_dominant'])
    if settings.show_sunrise_sunset:
        daily_params.extend(['sunrise','sunset'])
    if settings.show_daylight_duration: daily_params.append('daylight_duration')
    if settings.show_sunshine_duration: daily_params.append('sunshine_duration')
    if settings.show_daily_uv: daily_params.extend(['uv_index_max','uv_index_clear_sky_max'])
    if daily_params: params['daily'] = ','.join(list(set(daily_params)))
...
async def save_chat_to_db(session: AsyncSession, chat_id: int, chat_type: str, title: str = None) -> None:
    ...
    if not chat:
        chat = BotChat(chat_id=norm_id, chat_type=chat_type, title=title, is_active=True)
        session.add(chat)
        logger.info(f"Збережено новий чат {norm_id} ({chat_type})")
    else:
        chat.last_activity = datetime.now()
        chat.is_active = True
        if title and chat.title != title: chat.title = title
...
async def track_chat_member_update(
    ...
    norm_id = await normalize_chat_id(chat_id)
    chat = (await session.execute(select(BotChat).where(BotChat.chat_id==norm_id))).scalar_one_or_none()

    if new_status == 'member' and old_status != 'member':
        if not chat:
            chat = BotChat(chat_id=norm_id, chat_type=chat_type,
                           added_at=datetime.now(), last_activity=datetime.now(), is_active=True)
            session.add(chat)
        else:
            chat.is_active = True
            chat.last_activity = datetime.now()
...
async def update_setting(session: AsyncSession, telegram_id: int, key: str, value: str) -> None:
    settings = await get_user_weather_settings(session, telegram_id)
