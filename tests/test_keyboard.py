...
class TestPastDaysSelector:
    ...
    def test_all_options(self):
        ...
        assert (
            len(keyboard.inline_keyboard) == len(expected_days) + 1
        )  # +1 для кнопки назад
...
class TestTimezoneSelector:
    ...
    def test_all_timezones_present(self):
        ...
        callbacks = [
            row[0].callback_data.replace("set_timezone:", "")
...
class TestAdvancedDisplaySettings:
    ...
    def test_enabled_options(self):
        ...
        settings = {"show_dew_point": True, "show_solar_radiation": True}
        keyboard = WeatherKeyboards.advanced_display_settings(settings)

        enabled_count = sum(
            1 for row in keyboard.inline_keyboard if row and "✅" in row[0].text
        )
