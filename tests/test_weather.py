...
def test_format_datetime_date():
    assert WeatherFormatter.format_datetime("2024-06-01T12:34:56Z", "date") == "01.06.2024"
...
def test_format_datetime_datetime():
    assert WeatherFormatter.format_datetime("2024-06-01T12:34:56Z", "datetime") == "01.06.2024 12:34"

def test_format_datetime_weekday():
    # 2024-06-03 is Monday
    assert WeatherFormatter.format_datetime("2024-06-03T00:00:00Z", "weekday") == "Понеділок"
...
async def test_get_weather_request_error():
    params = {'hourly': 'temperature_2m'}
    with patch("httpx.AsyncClient.get", side_effect=httpx.RequestError("Network error", request=None)):
        with pytest.raises(WeatherAPIError):
            await WeatherService.get_weather(50.45, 30.52, params)
