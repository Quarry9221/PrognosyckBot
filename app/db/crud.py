# crud.py - CRUD операції для нової структури бази даних
...
async def get_or_create_user(
    ...
    if not user:
        ...
        # Створити дефолтні налаштування погоди
        await create_default_weather_settings(session, telegram_id)
    else:
        # Оновити інформацію користувача, якщо змінилася
        updated = False
...
async def create_default_weather_settings(session: AsyncSession, telegram_id: int) -> UserWeatherSettings:
    """Створити дефолтні налаштування погоди для користувача"""
...
async def update_user_units(
    ...
    if precipitation_unit and precipitation_unit in PRECIPITATION_UNITS:
...
async def toggle_display_setting(
    ...
    """Перемкнути налаштування відображення"""
    settings = await get_user_weather_settings(session, telegram_id)
...
# === ФУНКЦІЇ ДЛЯ ГЕНЕРАЦІЇ API ПАРАМЕТРІВ ===

async def get_api_parameters(session: AsyncSession, telegram_id: int) -> Dict[str, Any]:
    """Сформувати параметри для API запиту на основі користувацьких налаштувань"""
    settings = await get_user_weather_settings(session, telegram_id)
    ...
    # Додаємо elevation якщо встановлено
    ...
    if settings.show_solar_radiation:
        hourly_params.extend(['shortwave_radiation', 'direct_radiation', 'diffuse_radiation'])
    
    # Завжди додаємо weather_code та is_day
    hourly_params.extend(['weather_code', 'is_day'])
    
    if hourly_params:
        params['hourly'] = ','.join(list(set(hourly_params)))  # Видаляємо дублікати
    
    # Daily параметри
    daily_params = ['weather_code']  # Завжди включаємо weather_code
    ...
    # Current параметри (якщо ввімкнені)
...
async def track_chat_member_update(
    ...
    """Трекінг додавання/видалення бота з чату через ChatMemberHandler"""
    norm_id = await normalize_chat_id(chat_id)
    ...
    if new_status == 'member' and (old_status != 'member'):
        if not chat:
            ...
            logger.info(f"Бот додано до чату {norm_id} ({chat_type})")
        else:
            chat.is_active = True
            chat.last_activity = datetime.now()
            await session.commit()
            logger.info(f"Оновлено статус чату {norm_id} на активний")
...
async def get_user_settings_summary(session: AsyncSession, telegram_id: int) -> str:
    """Отримати зручний опис налаштувань користувача"""
    settings = await get_user_weather_settings(session, telegram_id)
...
async def get_settings(session: AsyncSession, telegram_id: int) -> dict:
    """Legacy функція для сумісності зі старим кодом"""
    settings = await get_user_weather_settings(session, telegram_id)
...
async def update_setting(session: AsyncSession, telegram_id: int, key: str, value: str) -> None:
    ...
    # Маппінг старих ключів на нові атрибути
