...
class TestUserWeatherSettingsModel:
    ...
    def test_create_settings(self, db_session):
        ...
        settings = UserWeatherSettings(
            user_id=123456789, latitude=50.4501, longitude=30.5234, location_name="Kyiv"
        )
    ...
    def test_settings_foreign_key_cascade(self, db_session):
        ...
        result = (
            db_session.query(UserWeatherSettings).filter_by(user_id=123456789).first()
        )
    ...
    def test_settings_all_boolean_flags(self, db_session):
        ...
        boolean_fields = [
            ...
            "show_visibility",
            "show_dew_point",
            "show_solar_radiation",
            "show_daily_temperature",
            "show_daily_precipitation",
            "show_daily_wind",
            "show_sunrise_sunset",
            "show_daylight_duration",
    ...
    def test_settings_notification_time_format(self, db_session):
        ...
        settings = UserWeatherSettings(user_id=123456789, notification_time="08:30")
        db_session.add(settings)
        db_session.commit()
...
class TestUserMessageModel:
    ...
    def test_message_nullable_user_id(self, db_session):
        ...
        message = UserMessage(chat_id=123456789, message_text="test")
        db_session.add(message)
        db_session.commit()
...
class TestBotChatModel:
    ...
    def test_create_chat(self, db_session):
        """Тест створення чату"""
        chat = BotChat(chat_id=123456789, chat_type="group", title="Test Group")
        db_session.add(chat)
        db_session.commit()
    ...
    def test_chat_default_values(self, db_session):
        ...
        chat = BotChat(chat_id=123456789, chat_type="private")
        db_session.add(chat)
        db_session.commit()
...
class TestDataIntegrity:
    ...
    def test_user_with_settings_relationship(self, db_session):
        ...
        settings = UserWeatherSettings(
            user_id=123456789, latitude=50.45, longitude=30.52
        )
        db_session.add(settings)
        db_session.commit()
        ...
        found_settings = (
            db_session.query(UserWeatherSettings)
    ...
    def test_multiple_messages_per_user(self, db_session):
        ...
        for i in range(5):
            message = UserMessage(
                user_id=123456789, chat_id=987654321, message_text=f"Message {i}"
            )
            db_session.add(message)
    ...
    def test_chat_activity_tracking(self, db_session):
        ...
        chat = BotChat(chat_id=123456789, chat_type="group", is_active=True)
        db_session.add(chat)
        db_session.commit()
...
class TestEdgeCases:
    ...
    def test_forecast_days_limits(self, db_session):
        ...
        settings = UserWeatherSettings(
            user_id=123456789, forecast_days=16, past_days=92  # максимум  # максимум
        )
        db_session.add(settings)
        db_session.commit()
    ...
    def test_long_location_name(self, db_session):
        ...
        settings = UserWeatherSettings(user_id=123456789, location_name=long_name)
        db_session.add(settings)
        db_session.commit()
    ...
    def test_negative_coordinates(self, db_session):
        ...
        settings = UserWeatherSettings(
            user_id=123456789, latitude=-33.8688, longitude=151.2093  # Сідней
        )
        db_session.add(settings)
        db_session.commit()
