...
sys.modules['bot.logger_config'] = types.SimpleNamespace(logger=types.SimpleNamespace(
...
async def test_geocode_place_success(monkeypatch):
    ...
    monkeypatch.setattr(httpx, "AsyncClient", lambda timeout: MockAsyncClient(response=MockResponse(json_data=mock_result)))
...
async def test_geocode_place_timeout(monkeypatch):
    monkeypatch.setattr(httpx, "AsyncClient", lambda timeout: MockAsyncClient(raise_timeout=True))
...
async def test_geocode_place_http_error(monkeypatch):
    monkeypatch.setattr(httpx, "AsyncClient", lambda timeout: MockAsyncClient(response=MockResponse(status_code=404)))
    with pytest.raises(ValueError, match="Помилка геокодування"):
        await geocode_place("Kyiv")

@pytest.mark.asyncio
async def test_geocode_place_network_error(monkeypatch):
    monkeypatch.setattr(httpx, "AsyncClient", lambda timeout: MockAsyncClient(raise_request_error=True))
